...
import { getRolePack } from "@anion/contracts";
import { keywordOverlap, sentenceSplit, summarizeText } from "@anion/shared/utils";
import type { SemanticScores } from "./signal-semantics";

export type SignalDimension = keyof typeof signalKeywords;
...
export type AnswerSignalProfile = {
  ...
  summary: string;
  /** Semantic similarity scores (0-1) per dimension, computed via embeddings. */
...
export function analyzeAnswerSignals(
  ...
  semantics?: { answerEmbedding: number[] },
): AnswerSignalProfile {
  ...
  const keywordTags = unique(tagHits.map((entry) => entry.tag));

  // Compute semantic tags if embedding is available
  let semanticScores: SemanticScores | undefined;
  let augmentedTags = keywordTags;
  if (semantics?.answerEmbedding) {
    // Lazy import to avoid circular dependency at module load time
    // (signal-semantics imports interview-director for SignalDimension type)
    const { computeSemanticScores } = require("./signal-semantics");
    const computed = computeSemanticScores(semantics.answerEmbedding);
    semanticScores = computed;

    // Augment tags: any dimension with cosine similarity > 0.65 is included
    const semanticTagEntries = (
      Object.entries(computed) as [SignalDimension, number][]
    ).filter(([, score]) => score > 0.65);
    augmentedTags = unique([...keywordTags, ...semanticTagEntries.map(([dim]) => dim)]);
  }
  ...
  const weaknesses = unique([
    ...
    ...(context.lastQuestion && relevance < 0.16
      ? ["回答偏离了问题的核心。"]
      : []),
    ...(!hasCausal && sentences.length > 2 ? ["回答缺乏清晰的因果链条。"] : []),
    ...(!hasEvidence && sentences.length > 3 && patternHits === 0 ? ["回答缺少具体案例或实例支撑。"] : []),
    ...(contradictionRisk ? ["回答表述过于确定，未证明边界条件。"] : []),
    ...(augmentedTags.includes("architecture") && !hasEvidence ? ["能否展开一个具体的架构决策？"] : []),
    ...(augmentedTags.includes("business") && !hasEvidence ? ["这个决策的可衡量影响是什么？"] : []),
    ...(augmentedTags.includes("process") && !hasEvidence ? ["实际落地时是如何处理交接的？"] : []),
  ]);

  const strengths = unique([
    ...(hasCausal ? ["回答包含可用的因果逻辑。"] : []),
    ...(hasEvidence ? ["回答尝试用证据自证。"] : []),
    ...(patternHits > 0 ? ["回答包含具体的项目经验。"] : []),
    ...(boundaryHits > 0 ? ["回答承认了重要的约束条件。"] : []),
    ...(augmentedTags.length > 0 ? [`候选人在${augmentedTags.slice(0, 2).join(" / ")}维度有深度。`] : []),
  ]);

  return {
    tags: augmentedTags.length > 0 ? augmentedTags : ["ownership"],
    weaknesses,
    strengths,
    relevance,
    causalHits,
