...
export async function resolveThread(params: {
  store: Pick<ApplicationStore, "getThread" | "createThread">;
  viewer: Viewer;
  mode: CommandMode;
  threadId?: string;
  sessionId?: string;
...
export async function runCommandMode(params: {
  ...
  const artifact = await params.ai.generateCommandArtifact({
    mode: params.mode,
...
export async function generateSandboxBeat(params: {
  ...
  const event = await params.ai.generateSandboxTurn({
    threadId: params.threadId,
