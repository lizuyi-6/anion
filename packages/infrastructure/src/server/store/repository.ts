...
import { hasOpenAi, runtimeEnv } from "@anion/config";
import type { ApplicationStore, SessionUpdate } from "@anion/application";
import type {
  ActiveMemoryContext,
  CommandArtifact,
  CommandMessage,
...
export interface DataStore extends ApplicationStore {
...
class MemoryDataStore implements DataStore {
  ...
  async uploadFile(file: File, options?: { kind?: UploadReference["kind"] }) {
    try {
      ...
      console.log(`鏂囦欢涓婁紶鎴愬姛: ${upload.originalName} (${upload.size} bytes)`);
      return upload;
    } catch (error) {
      console.error("MemoryDataStore 鏂囦欢涓婁紶澶辫触:", error);
      throw new Error(`鏂囦欢涓婁紶澶辫触: ${error instanceof Error ? error.message : "鏈煡閿欒"}`);
    }
  }
...
class SupabaseDataStore implements DataStore {
  ...
  private getUserIdForQuery(requestedUserId?: string) {
    if (this.isAdmin) {
      if (!requestedUserId) {
        throw new Error("Admin store access requires an explicit user ID.");
      }
      return requestedUserId;
    }

    if (!this.viewer) {
      throw new Error("An authenticated viewer is required.");
    }
  ...
  async uploadFile(file: File, options?: { kind?: UploadReference["kind"] }) {
    try {
      ...
      if (error) {
        console.error("Supabase 瀛樺偍涓婁紶澶辫触:", error);
        throw new Error(`鏂囦欢涓婁紶鍒板瓨鍌ㄦ湇鍔″け璐? ${error.message}`);
      }

      console.log(`鏂囦欢涓婁紶鎴愬姛: ${file.name} -> ${path}`);
      ...
      console.error("SupabaseDataStore 鏂囦欢涓婁紶澶辫触:", error);
      throw new Error(`鏂囦欢涓婁紶澶辫触: ${error instanceof Error ? error.message : "鏈煡閿欒"}`);
    }
  }
  ...
  async appendCommandMessage(message: CommandMessage) {
    ...
    if (!thread) {
      throw new Error(`鏈壘鍒颁細璇濈嚎绋嬶細${message.threadId}`);
    }
...
export async function getDataStore(options?: {
  ...
  if (runtimeEnv.dataDriver === "memory") {
  ...
  if (!options?.accessToken) {
    throw new Error("Supabase user store requires an access token.");
  }

  return new SupabaseDataStore(
    createSupabaseUserClient(options.accessToken),
    options.viewer ?? null,
    false,
  );
}
